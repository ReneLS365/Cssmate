...
async function loadTemplate (system, XLSX) {
  const path = TEMPLATE_PATHS[system]
  if (!path) throw new Error('Ukendt system: ' + system)

  const res = await fetch(path)
  if (!res.ok) throw new Error('Kunne ikke hente template: ' + path)

  const buf = await res.arrayBuffer()
  const wb = XLSX.read(buf, { type: 'array' })
  const wsName = wb.SheetNames[0]
  const ws = wb.Sheets[wsName]
  if (!ws) throw new Error('Mangler dataark i template: ' + path)
